...
parser.add_argument('--epochs', '-e', type=int, default=1, help='Number of sweeps over the dataset to train')
...
parser.add_argument('--num_randoms', type=int, default=3, help='No. of output perceptron (class)')
...
def makeDataForSmilesOnly(proteinName, dicC2I):
    ...
    train_x, valid_x, train_y, valid_y = train_test_split(train_x, train_y, test_size=0.1)#111)
...
def IGtest_step(inp_, real, whichClass):
    ...
    for i in range(args.num_randoms):
        randBaseFor1 = np.random.random_sample((args.seq_size, args.d_model))
        randBase = [randBaseFor1 for i in range(args.batchsize)]
        ...
        with tf.GradientTape() as g:
            g.watch(scaledXV)
            pred_logit, pred = encoder(scaledXV, whichClass, False, inp_padding_mask, justmask)
...
for tf1, tf2, tf3, tf4, tf5, tf6, tf7, tf8, tf9, tf10, tf11, tf12 in itertools.zip_longest(test_tf1, test_tf2, test_tf3, test_tf4, test_tf5, test_tf6,
                                     test_tf7, test_tf8, test_tf9, test_tf10, test_tf11, test_tf12):
    if tf1:
        igtest_res.append(IGtest_step(tf1[0], tf1[1], 0))
"""
    if tf2:
        test_step(tf2[0], tf2[1], 1)
    if tf3:
...
pickle.dump((igtest_res, indices2chars(test1, dicC2I)), open( "save_igtest1", "wb" ) )
