...
def run_model(model, inputTrain, yTrain, inputVal, yVal, inputTest, yTest, n=100, batch_size=32, model_file=None):
    ...
    tr = np.zeros((n, 1))#yTrain.shape[1]))
    val = np.zeros((n, 1))#yTrain.shape[1])) 
    tst = np.zeros((n, 1))#yTrain.shape[1])) 
...
def reduce_act(act, mols, nr_atoms=100):
    ...
    rep_idx = np.repeat(np.arange(len(mols)), atoms_per_mol)
    aa = []
    idx = []
